...
public class DataConverterUtil {
	...
	public static Object converterVO(Class<?> voClass) {
		...
		try { //
			...
			System.out.println("vo实例化时出现了错误，一般来说是系统错误");
			e.printStackTrace();
		}
		Field fields[] = voClass.getDeclaredFields(); // 获取全部的属性信息
		for (int x = 0; x < fields.length; x++) {
			try { //建议把异常捕捉放置在这里，因为即便是一个属性赋值失败的话 不会影响到vo类其他属性的设置
				String paramName = fields[x].getName();
				Method method = voClass.getMethod("set" + StringUtils.initcap(paramName), fields[x].getType());
				method.invoke(vo, converter(ServletObjectUtil.getParam().getParameter(paramName),fields[x].getType().getName()));
			} catch (Exception e) {
				// e.printStackTrace();
			}
		}
